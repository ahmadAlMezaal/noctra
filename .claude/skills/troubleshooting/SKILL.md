...
2. Check the startup banner in service logs; it prints the resolved backend, trigger mode, states or label, repo registry, review gate, auto-iterate, and notifications.
...
Config defaults to `~/.nightshift/` for `.env`, `repos.json`, and `logs/`, but a checkout containing `.env`, `repos.json`, `.env.example`, or `go.mod` uses the current directory.
...
4. The ticket has a Linear project that maps to `repos.json`, or `REPO_PATH` is set as a fallback.
...
Nightshift routes tickets by Linear project name. Confirm the project name exactly matches a key in `repos.json`:

```json
{
  "repos": {
    "Web App": { "url": "https://github.com/you/web-app.git", "main_branch": "main" }
  }
}
```

If the mapping is missing, Nightshift falls back to `REPO_PATH` when present. If neither exists, it skips the ticket with a Linear comment.
