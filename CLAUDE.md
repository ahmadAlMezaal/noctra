...
- After re-engaging, Noctra replies to the PR's unresolved review threads (`github.ReplyToThreads`), carrying the agent's own summary (what it addressed, what it pushed back on) extracted from the run log via `agent.ExtractSummary`, plus the hidden `github.NoctraReplyMarker` so the watcher won't re-read its own reply: "Addressed in `<sha>`." + the summary when HEAD advanced during the iteration (whether Noctra pushed or the agent committed+pushed itself — keyed on HEAD movement, **not** branch-ahead-of-remote, since the agent sometimes self-pushes), or the agent's reasoning for why no change was needed when HEAD didn't move (so a no-diff review isn't silent on GitHub). **It does not resolve the threads** (`resolve=false`): an iteration bundles review feedback + CI and one run-summary can't prove each individual thread's finding was addressed, so auto-resolving would silently close untouched findings (observed on PR #246, where two distinct Gemini `index.html` findings were marked resolved by a reply that only described an unrelated `gofmt` fix). Threads are left open for a human; the `resolve` flag is the seam for a future per-finding reply that resolves only what it actually addressed. The summary is also persisted to `state.PRState.LastReasoning` and fed into the next iteration's fix prompt (`agent.FixPromptInput.PriorReasoning`) so re-engagements don't re-litigate settled feedback. It also notifies via Telegram/Linear.
