...
| Package | Purpose |
|---------|---------|
| `cmd/nightshift` | Entry point + subcommand dispatch (`run` / `setup` / `cleanup` / `doctor` [`--json`] / `update` / `logs` / `start` / `stop` / `restart` / `status` / `completion` / `version`); `start`/`stop`/`restart`/`status` are thin `systemctl --user <verb> nightshift.service` wrappers (status also prints the binary version; missing-systemctl hint mirrors `logs`/`journalctl`), `completion bash\|zsh` prints a static shell-completion script (pure `completionScript` fn, unit-tested); startup banner; `--help` |
