...
## Dashboard frontend (Vite + Preact)

The operations dashboard lives in `internal/dashboard`. The UI is a **Vite + Preact + TypeScript** project under `internal/dashboard/web/` (`src/components/` = one component per panel: KPIs, active runs, queue, run history, donut, token/cost, spend, runs-by-repo, throughput, budget, repo cards, sweep matrix, log overlay). It is **built into a single self-contained `index.html`** and committed to `internal/dashboard/static/`, which `dashboard.go` serves via `//go:embed static`.

**Why a single inlined file** (`vite-plugin-singlefile` in `web/vite.config.ts`): the page is served behind a read token, and `@font-face`/`<script>`/`<link>` subrequests don't carry the page's `?token=` query param. The old dashboard sidestepped this by being one inline-everything HTML file, with `/fonts/` carved out of auth (`mux.Handle("/fonts/", …)`). Inlining all JS + CSS into the HTML preserves that exact model: the only subresource is fonts, and they stay unauthenticated. **Do not switch Vite to emit separate `/assets/*.js|css` chunks** — they'd hit the token gate and 401, and the page would silently fail to boot. Fonts live in `web/public/fonts/` (source of truth) → copied to `static/fonts/` at build, referenced by the absolute `/fonts/` URL so Vite leaves them external.

**Release pipeline — built output is committed (option B).** `go build ./...` embeds `static/`, so the directory must contain real files to compile, and `internal/dashboard/dashboard_test.go` serves the page under `go test ./...` with no Node. Building in CI only (option A) would require a Node step in front of every `go build`/`go test`, breaking the no-Node dev/build flow. Committing the build output keeps CI, the release workflow (`tag-on-merge.yml` → GoReleaser), and the Dockerfile **pure Go — no Node step anywhere**. The trade-off: generated assets live in git, so **after changing anything under `web/`, rebuild and commit `static/`** (CI does not regenerate it):
...
`web/node_modules/` is gitignored; the committed `static/index.html` is the artifact. Keep the port faithful — the live `index.html` in git history is the source of truth for exact visuals/derivations.
