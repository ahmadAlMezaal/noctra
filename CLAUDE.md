...
| `internal/linear` | Linear GraphQL client: `ResolveStateIDs`, `FetchTriggerIssues`, `FetchLabeledIssues` (both fetch each issue's `comments` so human clarifications reach the agent — see `Issue.ClarificationComments`, which filters out Noctra's own automated notices; project descriptions are fetched too, parsed by `Project.RepoDirective` for `Repo:`/`Branch:` routing), `ResolveLabelID`, `RemoveLabel`, `SetState`, `Comment`; read queries for Telegram — `ProjectIssueCounts`, `ListProjectIssues`, `SearchIssues`, `GetIssueByIdentifier`. Auth is a personal API key (`New`) sent verbatim, or an app-actor OAuth token (`NewOAuth`, sent as `Bearer`) when `LINEAR_OAUTH_TOKEN` is set — the latter attributes comments/state changes to the **Noctra app** instead of the personal user (`actor=app`; `pipeline.newLinearClient` picks one). The `ClarificationComments` self-comment filter is body-based (`"**Noctra"` prefix), so it's unaffected by the actor. |
