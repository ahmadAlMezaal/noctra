...
func TestRunManual_CopiesEnvTemplate(t *testing.T) {
	...
	// Manual setup only copies .env — repos come from the Linear Repo: directive, no registry file.
...
func TestAskInt_EOFPreservesExisting(t *testing.T) {
	// On EOF, existing "7" should win over factory default 3.
	w := newWizardWithInput("")
...
func TestMergeEnvFile_PreservesUnknownKeys(t *testing.T) {
	...
	// A .env with hand-added keys the wizard doesn't manage (the ENG-214 scenario).
